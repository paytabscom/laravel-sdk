...
the first argument, and always signal failure with `CallbackProcessingException`.
...
`InvalidPayloadException` and `IdempotencyException` now extend `CallbackProcessingException`
rather than `RuntimeException`. Catching `CallbackProcessingException` covers all three.
